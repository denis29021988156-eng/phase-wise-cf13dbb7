...
const Profile = () => {
  ...
  const checkCalendarTokens = async () => {
    if (!user) return;
    ...
    try {
      ...
      const { data: microsoftData } = await supabase
        ...
        .maybeSingle();
      setHasMicrosoftToken(!!microsoftData);
              ...
              <div className="flex items-center space-x-3">
                <div className="p-2 rounded-full bg-blue-100">
                  <svg className="h-5 w-5 text-blue-600" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M24 7.387v9.226a1.39 1.39 0 01-1.39 1.39h-7.177a1.39 1.39 0 01-1.39-1.39V7.387a1.39 1.39 0 011.39-1.39h7.177a1.39 1.39 0 011.39 1.39zM0 15.52V8.48A.48.48 0 01.48 8h11.04a.48.48 0 01.48.48v7.04a.48.48 0 01-.48.48H.48A.48.48 0 010 15.52z"/>
                  </svg>
                </div>
                <div>
                  <p className="font-medium">{t('profile.outlookCalendar')}</p>
                  <p className="text-sm text-muted-foreground">{t('profile.connected')}</p>
                </div>
              </div>
              <div className="w-3 h-3 rounded-full bg-green-500"></div>
          ...
          {hasAppleToken && (
            <div className="flex items-center justify-between p-4 rounded-lg bg-muted/50">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="font-medium">{t('profile.appleCalendar')}</p>
                  <p className="text-sm text-muted-foreground">{t('profile.connected')}</p>
                </div>
              </div>
              <div className="w-3 h-3 rounded-full bg-green-500"></div>
